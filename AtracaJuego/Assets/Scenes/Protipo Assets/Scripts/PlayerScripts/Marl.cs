...
public class Marl : PlayablePlayer
{
    ...
    public override void ChangeMapShown(int setMode){
        ...
        else if (Mode == 2) { GC.setAttackPos(transform.position, 1, true, true, false, 2, false); GC.setReachablePos(transform.position, MaxDistance, true, 1, true, true); changeCircle(true); }
        else
        {
            changeCircle(false);
            GC.setAttackPos(transform.position, 1, true, true, false, 2, true); GC.setReachablePos(transform.position, MaxDistance, true, 1, true, true);
        }
