            ...
            if(!GC.isEmpty(posMouse, false, 2)){
                ...
                if(Mathf.Abs(x)!=Mathf.Abs(y)){ //A los lados
                    switch(Mathf.Abs(x)>0){
                        case false: if(y>0){   ElecPrefab.GetComponent<ElecEffect>().direction = 3;
                                            Instantiate(ElecPrefab, posNew, Quaternion.identity);}
                                else{       ElecPrefab.GetComponent<ElecEffect>().direction = 4;
                                            Instantiate(ElecPrefab, posNew, Quaternion.identity);
                                } break;
                        case true: if(x>0){   ElecPrefab.GetComponent<ElecEffect>().direction = 1; 
                                            Instantiate(ElecPrefab, posNew, Quaternion.identity);}
                                else{       ElecPrefab.GetComponent<ElecEffect>().direction = 2; 
                                            Instantiate(ElecPrefab, posNew, Quaternion.identity);
                                }break;
                    }
                }
